...
- `cats()` — the 14 infrastructure categories (id, color, bilingual name). 10 are stackable layers; `seguridad`, `cicd`, `iac`, `monitorizacion` are cross-cutting (rendered separately, not in the stack).
...
The app has **six** modes, switched from the header toggle and driven by `state.mode`. Product-level status and roadmap live in [ROADMAP.md](ROADMAP.md).
