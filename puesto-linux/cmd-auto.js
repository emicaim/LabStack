...
(function (P) {
'use strict';
const U = P.u, L = U.L, H = U.HOME, INFRA = H + '/infra', TFDIR = INFRA + '/terraform';
...
const INVENTARIO = ['# Inventario de la plataforma retail. Los grupos mandan: los playbooks', '# apuntan a grupos, nunca a máquinas sueltas.', '', '[bastion]', 'bastion', '', '[controllers]', 'ctl01', 'ctl02', 'ctl03', '', '[compute]', 'cmp01', 'cmp02', 'cmp03', '', '# Hipervisores recién montados, pendientes de alta (alta-compute.yml)', '[compute_nuevos]', 'cmp04', '', '[ceph]', 'ceph01', 'ceph02', 'ceph03', '', '[monitoring]', 'mon01', '', '[openstack:children]', 'controllers', 'compute'];
...
const nada = () => 'ok';
const PB = {};
PB['chrony.yml'] = { plays: [{ nombre: 'Sincronización horaria (chrony)', hosts: 'all', tareas: [
  { n: 'chrony instalado', f: nada },
  { n: 'configuración de chrony (servidores NTP internos)', f: nada },
  { n: 'chrony arrancado y habilitado', f: asegurar('chrony') },
  { n: 'forzar sincronización si el desfase supera 50 ms', f: (st, h, c) => { if (Math.abs(h.ntp.offset) < 0.05) return 'skipped'; if (c.check) return 'skipped'; h.ntp.offset = 0.000011; return 'changed'; } },
] }], yaml: ['---', '# Hora sincronizada en toda la flota. Ceph protesta en cuanto un monitor se', '# desvía más de 50 ms, y los tokens de Keystone caducan mal con relojes locos.', '- name: Sincronización horaria (chrony)', '  hosts: all', '  become: true', '  tasks:', '    - name: chrony instalado', '      ansible.builtin.apt:', '        name: chrony', '        state: present', '', '    - name: configuración de chrony (servidores NTP internos)', '      ansible.builtin.template:', '        src: templates/chrony.conf.j2', '        dest: /etc/chrony/chrony.conf', '      notify: reiniciar chrony', '', '    - name: chrony arrancado y habilitado', '      ansible.builtin.service:', '        name: chrony', '        state: started', '        enabled: true', '', '    - name: forzar sincronización si el desfase supera 50 ms', '      ansible.builtin.command: chronyc makestep', '      when: clock_offset | float | abs > 0.05', '', '  handlers:', '    - name: reiniciar chrony', '      ansible.builtin.service:', '        name: chrony', '        state: restarted'] };

PB['logrotate.yml'] = { plays: [{ nombre: 'Logs de OpenStack y RabbitMQ bajo control', hosts: 'controllers', tareas: [
  { n: 'nivel de log de RabbitMQ en info', notify: 'reiniciar rabbitmq', f: (st, h, c) => { if (!h.extra.rabbitDebug) return 'ok'; if (!c.check) h.extra.rabbitDebug = false; return 'changed'; } },
  { n: 'política de rotación de OpenStack y RabbitMQ', f: nada },
  { n: 'rotar y comprimir los logs que pasan de 1 GB', f: (st, h, c) => {
...
], handlers: { 'reiniciar rabbitmq': (st, h, c) => { if (c.check) return 'changed'; const r = P.reiniciarUnidad(st, h, 'rabbitmq-server'); return r ? { fallo: 'Unable to restart service rabbitmq-server: Job for rabbitmq-server.service failed because the control process exited with error code.' } : 'changed'; } } }],
yaml: ['---', '# Logs de los controladores: nivel razonable y rotación diaria.', '# Un log en debug olvidado llena /var en horas.', '- name: Logs de OpenStack y RabbitMQ bajo control', '  hosts: controllers', '  become: true', '  tasks:', '    - name: nivel de log de RabbitMQ en info', '      ansible.builtin.lineinfile:', '        path: /etc/rabbitmq/rabbitmq.conf', "        regexp: '^log.file.level'", "        line: 'log.file.level = info'", '      notify: reiniciar rabbitmq', '', '    - name: política de rotación de OpenStack y RabbitMQ', '      ansible.builtin.template:', '        src: templates/logrotate-openstack.j2', '        dest: /etc/logrotate.d/openstack', '', '    - name: rotar y comprimir los logs que pasan de 1 GB', '      ansible.builtin.command: logrotate -f /etc/logrotate.d/openstack', '      when: logs_grandes | length > 0', '', '  handlers:', '    - name: reiniciar rabbitmq', '      ansible.builtin.service:', '        name: rabbitmq-server', '        state: restarted'] };

PB['nova-compute.yml'] = { plays: [{ nombre: 'Hipervisores (nova-compute + libvirt)', hosts: 'compute', tareas: [
  { n: 'paquetes de nova-compute, libvirt y Open vSwitch', f: nada },
  { n: 'nova.conf desde plantilla', f: nada },
  { n: 'libvirtd arrancado y habilitado', f: asegurar('libvirtd') },
  { n: 'nova-compute arrancado y habilitado', f: asegurar('nova-compute') },
  { n: 'Open vSwitch arrancado y habilitado', f: asegurar('openvswitch-switch') },
  { n: 'agente de Open vSwitch arrancado y habilitado', f: asegurar('neutron-openvswitch-agent') },
] }], yaml: ['---', '# Estado deseado de los hipervisores. Idempotente: si todo está bien, no cambia nada.', '- name: Hipervisores (nova-compute + libvirt)', '  hosts: compute', '  become: true', '  tasks:', '    - name: paquetes de nova-compute, libvirt y Open vSwitch', '      ansible.builtin.apt:', '        name: [nova-compute, libvirt-daemon-system, neutron-openvswitch-agent]', '        state: present', '', '    - name: nova.conf desde plantilla', '      ansible.builtin.template:', '        src: templates/nova-compute.conf.j2', '        dest: /etc/nova/nova.conf', '', '    - name: libvirtd arrancado y habilitado', '      ansible.builtin.service: {name: libvirtd, state: started, enabled: true}', '', '    - name: nova-compute arrancado y habilitado', '      ansible.builtin.service: {name: nova-compute, state: started, enabled: true}', '', '    - name: Open vSwitch arrancado y habilitado', '      ansible.builtin.service: {name: openvswitch-switch, state: started, enabled: true}', '', '    - name: agente de Open vSwitch arrancado y habilitado', '      ansible.builtin.service: {name: neutron-openvswitch-agent, state: started, enabled: true}'] };

PB['alta-compute.yml'] = { plays: [{ nombre: 'Alta de hipervisores nuevos', hosts: 'compute_nuevos', tareas: [
  { n: 'repositorio Ubuntu Cloud Archive (OpenStack 2024.1 Caracal)', f: (st, h) => h.provisionado ? 'ok' : 'changed' },
  { n: 'paquetes de nova-compute, libvirt y Open vSwitch', f: (st, h, c) => { if (h.provisionado) return 'ok'; if (!c.check) P.provisionar(st, h); return 'changed'; } },
  { n: 'nova.conf y neutron desde plantilla', f: (st, h, c) => { if (h.extra.configurado) return 'ok'; if (!c.check) h.extra.configurado = true; return 'changed'; } },
  { n: 'servicios arrancados y habilitados', f: (st, h, c) => {
...
PB['ceph-reboot.yml'] = { plays: [
  { nombre: 'Preparar el clúster Ceph', hosts: 'ceph', unaVez: true, tareas: [
    { n: 'el clúster está en HEALTH_OK', cmd: true, f: (st) => { const s = P.saludCeph(st), solo = s.checks.every(c => c.code === 'OSDMAP_FLAGS' && /^noout flag/.test(c.res)); return (s.estado === 'HEALTH_OK' || solo) ? 'ok' : { fallo: '{"changed": true, "cmd": ["ceph", "health"], "failed_when_result": true, "stdout": "' + s.estado + ' ' + s.checks.map(c => c.res).join('; ') + '"}', crudo: true }; } },
  ...
  { nombre: 'Reinicio ordenado de nodos Ceph', hosts: 'ceph', serial: 1, tareas: [
    { n: 'reiniciar el nodo', f: (st, h, c) => { if (c.check) return 'changed'; P.reiniciar(st, h); U.hasta(st, () => h.up, 30); return 'changed'; } },
    { n: 'esperar a que sus OSD estén up', cmd: true, f: (st, h) => { U.hasta(st, () => h.osds.every(i => U.osdUp(st, st.ceph.osds[i])), 5); return 'ok'; } },
    { n: 'esperar a que el clúster se recupere', cmd: true, f: (st) => { U.hasta(st, () => !st.ceph.rec, 15); return 'ok'; } },
  ] },
  { nombre: 'Devolver el clúster a la normalidad', hosts: 'ceph', unaVez: true, tareas: [
  ...
  for (const play of pb.plays) {
    ...
    for (const lote of lotes) {
      ...
      for (const t of play.tareas) {
        ...
        activos.forEach(n => {
          const h = st.hosts[n];
          let r = (check && t.cmd && !/esperar/.test(t.n) && t.n.indexOf('HEALTH_OK') < 0) ? 'skipped' : t.f(st, h, { check });
...
const sinInventario = [L('[WARNING]: No inventory was parsed, only implicit localhost is available', 'ambar'), L('[WARNING]: provided hosts list is empty, only localhost is available. Note that the implicit localhost does not match \'all\'', 'ambar'), L('(ansible.cfg y el inventario viven en ~/infra: haz cd ~/infra y repite)', 'dim')];

U.cmd('ansible-playbook', { ayuda: 'lanza un playbook (-l para limitar, --check para ensayar)', grupo: 'Automatización', donde: ['bastion'], fuera: '(Ansible se lanza desde el bastión, en ~/infra)', fn: ctx => {
  ...
  if (ruta.indexOf(INFRA + '/playbooks/') !== 0 || !PB[nombre]) return [L('ERROR! the playbook: ' + pos[0] + ' could not be found', 'err'), L('(los playbooks están en ~/infra/playbooks: ls ~/infra/playbooks)', 'dim')];
  ...
  if (check) r.lineas.unshift(L('(modo --check: Ansible cuenta lo que cambiaría, sin tocar nada)', 'dim'));
...
U.cmd('ansible', { ayuda: 'órdenes sueltas a la flota (ansible all -m ping)', grupo: 'Automatización', donde: ['bastion'], fuera: '(Ansible se lanza desde el bastión, en ~/infra)', fn: ctx => {
  const st = ctx.st, a = ctx.args;
...
U.cmd('ansible-inventory', { ayuda: 'grupos y máquinas del inventario (--graph)', grupo: 'Automatización', donde: ['bastion'], fn: ctx => {
...
U.cmd('terraform', { ayuda: 'infraestructura como código: plan / apply (en ~/infra/terraform)', grupo: 'Automatización', donde: ['bastion'], fuera: '(Terraform se lanza desde el bastión, en ~/infra/terraform)', fn: ctx => {
  ...
  if (sub === 'destroy') return [L('(terraform destroy borraría los grupos de seguridad de producción: en el simulador está desactivado)', 'dim')];
  if (sub !== 'plan' && sub !== 'apply') return [L('Terraform has no command named "' + sub + '".', 'err')];
  if (!ses.env.OS_AUTH_URL) return tfError(['Error: One of \'auth_url\' or \'cloud\' must be specified', '', '  with provider["registry.terraform.io/terraform-provider-openstack/openstack"],', '  on main.tf line 10, in provider "openstack":', '  10: provider "openstack" {', '']).concat([L('(el provider lee las credenciales del entorno: source ~/admin-openrc)', 'dim')]);
...
// ------------------------------------------------------------------ Alertmanager
U.cmd('amtool', { ayuda: 'alertas activas en Alertmanager (amtool alert)', grupo: 'Monitorización', donde: ['bastion', 'mon'], fn: ctx => {
  const st = ctx.st, a = ctx.args;
  if (a[0] !== 'alert') return [L('usage: amtool [<flags>] <command> [<args> ...]', 'err'), L('(en el simulador: amtool alert)', 'dim')];
...
const MAIN_TF = ['terraform {', '  required_version = ">= 1.6"', '  required_providers {', '    openstack = {', '      source  = "terraform-provider-openstack/openstack"', '      version = "~> 2.1"', '    }', '  }', '}', '', 'provider "openstack" {', '  # Credenciales desde el entorno (source ~/admin-openrc)', '}'];
const SG_TF = ['# Grupo de seguridad del backend de los TPV de tienda.', '# Todo lo que no esté aquí no debería existir en OpenStack.', '', 'resource "openstack_networking_secgroup_v2" "pos_backend" {', '  name        = "sg-pos-backend"', '  description = "Backend TPV (gestionado por Terraform)"', '  tenant_id   = var.proyecto_tpv', '}', '', '# HTTPS público: las tiendas llegan por internet.', '', 'resource "openstack_networking_secgroup_rule_v2" "https_public" {', '  direction         = "ingress"', '  ethertype         = "IPv4"', '  protocol          = "tcp"', '  port_range_min    = 443', '  port_range_max    = 443', '  remote_ip_prefix  = "0.0.0.0/0"', '  security_group_id = openstack_networking_secgroup_v2.pos_backend.id', '}', '', '# SSH sólo desde la red de administración.', 'resource "openstack_networking_secgroup_rule_v2" "ssh_admin" {', '  direction         = "ingress"', '  ethertype         = "IPv4"', '  protocol          = "tcp"', '  port_range_min    = 22', '  port_range_max    = 22', '  remote_ip_prefix  = "10.20.0.0/16"', '  security_group_id = openstack_networking_secgroup_v2.pos_backend.id', '}', '', '# API interna de sincronización, sólo desde la red de TPV.', 'resource "openstack_networking_secgroup_rule_v2" "api_internal" {', '  direction         = "ingress"', '  ethertype         = "IPv4"', '  protocol          = "tcp"', '  port_range_min    = 8443', '  port_range_max    = 8443', '  remote_ip_prefix  = "10.30.0.0/16"', '  security_group_id = openstack_networking_secgroup_v2.pos_backend.id', '}'];
P.ficheros.push((st, h) => {
  ...
  f[INFRA + '/README.md'] = fijo(['# infra', '', 'playbooks/   Ansible: estado deseado de la flota', 'inventory/   quién es quién', 'terraform/   recursos de OpenStack declarados (grupos de seguridad)', '', 'Lanza Ansible desde esta carpeta (aquí está ansible.cfg).']);
  ...
  f[TFDIR + '/variables.tf'] = fijo(['variable "proyecto_tpv" {', '  description = "ID del proyecto tpv-tiendas"', '  type        = string', '  default     = "' + U.hex('projtpv-tiendas', 32) + '"', '}']);
  return f;
});
