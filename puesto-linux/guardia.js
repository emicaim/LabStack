...
P.familias = [
  { id: 'osd-caido', nivel: 1, titulo: 'Un OSD que se cuelga',
    leccion: 'un OSD caído se levanta antes de que Ceph lo marque out (10 minutos), y su RECENT_CRASH se lee y se archiva',
  ...
  { id: 'libvirt-parado', nivel: 1, titulo: 'libvirt parado en un hipervisor',
    leccion: 'sin libvirt no hay nova-compute: primero la base, luego lo que depende de ella, y habilitado para que sobreviva a un reinicio',
    crear: rnd => [{ tipo: 'servicio-parado', nodo: uno(rnd, CMP), servicio: 'libvirtd', hace: entre(rnd, 30, 900), deshabilitado: true }] },
  { id: 'galera-oom', nivel: 1, titulo: 'Un nodo de Galera muerto por falta de memoria',
    leccion: 'a un clúster Galera vivo se vuelve con un arranque normal; galera_new_cluster sólo con todo el clúster parado',
    crear: rnd => [{ tipo: 'servicio-caido', nodo: uno(rnd, CTL), servicio: 'mariadb', motivo: 'oom', hace: entre(rnd, 5, 60) }] },
  { id: 'reloj', nivel: 1, titulo: 'Un reloj que deriva',
    leccion: 'chrony parado y deshabilitado deja la hora a la deriva; makestep la corrige ya, el slew tarda',
    crear: rnd => [{ tipo: 'reloj-desfasado', nodo: uno(rnd, NODOS), segundos: +(0.12 + rnd() * 0.8).toFixed(3), hace: entre(rnd, 60, 600) }] },
  { id: 'noout-olvidado', nivel: 1, titulo: 'Un noout olvidado',
    leccion: 'los flags de mantenimiento se quitan al terminar: noout impide que Ceph reaccione solo',
    crear: rnd => [{ tipo: 'flag-ceph', flag: 'noout', hace: entre(rnd, 300, 1440) }] },
  { id: 'ovs-caido', nivel: 2, titulo: 'Open vSwitch caído en un hipervisor',
    leccion: 'una VM puede estar ACTIVE y sin red: Open vSwitch antes que el agente que lo usa',
    crear: rnd => [{ tipo: 'servicio-caido', nodo: uno(rnd, CMP), servicio: 'openvswitch-switch', motivo: 'ovs-lock', hace: entre(rnd, 5, 60) }] },
  { id: 'cinder-atascado', nivel: 2, titulo: 'cinder-volume caído y un borrado a medias',
    leccion: 'un estado que no avanza es un servicio que murió a mitad: primero se levanta el servicio, luego se corrige el estado',
  ...
  { id: 'rabbit-lleno', nivel: 2, titulo: 'RabbitMQ sin disco',
    leccion: 'un disco lleno es un síntoma: du encuentra al culpable antes de borrar nada',
  ...
  { id: 'nearfull', nivel: 2, titulo: 'Un OSD casi lleno con el balancer apagado',
    leccion: 'Ceph se llena por el OSD más lleno, no por la media; se reparte, no se sube el umbral',
    crear: rnd => [{ tipo: 'balancer-apagado', osd: entre(rnd, 0, 8), exceso: +(28.5 + rnd() * 2.5).toFixed(1) }] },
  { id: 'certificado', nivel: 2, titulo: 'El certificado de la API caducado',
    leccion: 'el fichero en disco y lo que el servicio tiene cargado son dos cosas: tras cambiarlo, se recarga',
    crear: rnd => [{ tipo: 'certificado-caducado', hace: entre(rnd, 5, 120) }] },
  { id: 'fuga', nivel: 2, titulo: 'Una fuga de memoria en nova-api',
    leccion: 'reiniciar mitiga, el parche corrige, y un paquete instalado no cambia el proceso que ya corre',
    crear: rnd => { const m = {}; CTL.forEach(n => { m[n] = entre(rnd, 8, 30); }); m[uno(rnd, CTL)] = entre(rnd, 60, 64); return [{ tipo: 'fuga-memoria', memoria: m }]; } },
  { id: 'disco-muerto', nivel: 3, titulo: 'Un disco muerto y un noout que lo tapa',
    leccion: 'un disco con SMART FAILED no se reinicia: se saca el OSD; y noout olvidado impide que Ceph se cure solo',
    crear: rnd => [{ tipo: 'flag-ceph', flag: 'noout', hace: entre(rnd, 300, 900) }, { tipo: 'disco-muerto', osd: entre(rnd, 0, 8), hace: entre(rnd, 30, 200) }] },
  { id: 'quorum', nivel: 3, titulo: 'Dos monitores de Ceph caídos',
    leccion: 'un clúster de tres aguanta un fallo, no dos; el primero, ignorado, es el que prepara la caída',
  ...
  { id: 'mon-disco', nivel: 3, titulo: 'debug_mon en 20 y un monitor sin disco',
    leccion: 'la configuración central afecta a todos los demonios del tipo: el síntoma sale en un nodo y la causa vive en el clúster',
  ...
  { id: 'regla-a-mano', nivel: 3, titulo: 'Un puerto abierto a mano, fuera de Terraform',
    leccion: 'Terraform sólo ve lo que tiene en su estado: lo creado a mano hay que encontrarlo y quitarlo a mano',
    crear: () => [{ tipo: 'regla-a-mano' }] },
];

const NOMBRES = { 1: 'Tranquila', 2: 'Movida', 3: 'Infierno' };
...
P.generarGuardia = function (nivel, semilla) {
  ...
  for (let intento = 0; intento < 25; intento++) {
    ...
    const def = {
      ...
      tipo: 'incidente', asunto: 'Guardia ' + NOMBRES[nivel].toLowerCase() + ' · semilla ' + semilla,
      de: 'Busca de guardia', cuerpo: ['Te ha saltado el busca. No hay ticket: sólo lo que ven las alertas.'],
      impacto: nivel === 1 ? 'medio' : 'alto', urgencia: 'alta', nivel, hace: 30, abiertoHace: 0,
      averias,
      causa: 'Pasaban ' + (familias.length === 1 ? 'una cosa' : familias.length + ' cosas a la vez') + ': ' + familias.map(f => f.titulo.charAt(0).toLowerCase() + f.titulo.slice(1)).join('; ') + '.',
